...
The macOS app exposes a dedicated **Cloud Sync** sidebar screen with Apple Account/iCloud Drive status, a System Settings sign-in action, secure passphrase fields, optional printable recovery keys, iCloud sync/import, manual encrypted backup export/import, timestamped backup history, retention, scheduled backups while the unlocked app is running, and restore comparison metadata. Installing a recovery key also wraps the local vault master key so it can be restored after Keychain loss; `vibevault recovery status|restore` provides the break-glass CLI flow. The encrypted local vault retains up to 50 revisions per secret, with single-secret restore and Recently Deleted recovery. See `docs/CLOUD_SYNC_AND_BACKUP.md` for the full workflow.
